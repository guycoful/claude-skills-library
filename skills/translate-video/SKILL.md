...
**Uses word-level timestamps via sequential index tracking — never text search.**

##### Step-by-step workflow:

**3.1 — Plan subtitle groups (read `.md` only, do NOT read full JSON)**

Read `.md` to understand the full text. Decide how to group words into natural sentences/clauses. Write a plan as a list of groups, each with:
- The English text of that group (consecutive words)
- How many words it contains

Example plan:
```
Group 1 (16 words): "All right, let's talk about a huge step forward in making our platform safer for everyone."
Group 2 (7 words): "Honestly, this was a huge blind spot."
...
```

**3.2 — Build timestamps using sequential index cursor**

Run this Python script. It loads the JSON once, then advances a cursor sequentially — no text search, no ambiguity:

```python
import json, re

WORDS_JSON = 'PATH_TO_transcript.json'
OUTPUT_SRT = 'PATH_TO_output_he.srt'

def ms(s):
    s = round(s * 1000)
    h=s//3600000; s%=3600000; m=s//60000; s%=60000; sec=s//1000; ms=s%1000
    return f'{h:02d}:{m:02d}:{sec:02d},{ms:03d}'

with open(WORDS_JSON) as f:
    words = json.load(f)['words']

# FILL THIS IN: list of word counts per subtitle group, in order
# Each number = how many consecutive words belong to that subtitle
GROUP_SIZES = [
...
for size in GROUP_SIZES:
    ...
    start_ts = ms(group[0]['start'])
    end_ts   = ms(group[-1]['end'])
    text = ' '.join(w['word'] for w in group)
    entries.append((start_ts, end_ts, text))
    cursor += size

# Write SRT (text will be replaced with translation below)
lines = []
...
print(f'{len(entries)} entries, cursor ended at {cursor}/{len(words)} words')
```

**3.3 — Replace English text with translated Hebrew**

After running the script, edit the SRT to replace each English group with its Hebrew translation. The timestamps stay exactly as computed.
